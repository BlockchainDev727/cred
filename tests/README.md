...
Core tests take longer than any other Cred tests, due to the high amount of computational work involved in validating core components.
...
To run only Cred's core tests (after building):
...
To run only Cred's crypto tests (after building):
...
```
credd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `cred-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Cred's hash tests (after building):
...
To run only Cred's performance tests (after building):
...
To run only Cred's unit tests (after building):
